...
§3.3 조건부 재정의 논리(**ACK 없음 → 안전 상태**)는 A3 이하에서 올바르다. A4의 치명적이거나 돌이킬 수 없는 결정의 경우, 안전 상태는 어떠한 조건 하에서도 — 운영자 부재, 네트워크 장애, 시간 압박, 또는 해당 행동이 비치명적이고 가역적이라는 인증된 WA 결정을 제외한 어느 원칙 계층의 명시적 지시를 포함하여 — 허용 가능한 *유일한* 결과이다.
...
자율성 등급 변경에 대한 WA 검토 통합의 운용 증거는 CIRISAgent `compliance/` 디렉토리 (차원 D22/D23)에 있다.

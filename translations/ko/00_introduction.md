...
> CIRIS 1.3-RC1은 출시 후보(Release Candidate) ASI 정렬 제안으로, 적대적 검토에 개방되어 있습니다. 본문은 완성되어 있으며 스텁(stub) 섹션은 남아 있지 않습니다. 수학적 내용은 공식 산출물 인용을 포함하며, 부록 F–I는 운용화되었습니다. 최종 지위는 부록들의 실가동 주기 검증, Book IX 경험적 검증 프로그램 완료, 그리고 전면적인 레드팀 훈련이 완료된 이후에 결정됩니다. 수치 임계값, 지연 목표, 거버넌스 할당량은 현재 활발히 검토 중입니다.

# CIRIS 협약 버전 1.3-RC1 — 출시 후보 ASI 정렬 제안 (적대적 검토 개방)
...
**현재 상태**: 출시 후보 (v1.3-RC1)

RC 지위는 **본문 완성도**를 반영합니다. 모든 섹션은 운용화된 내용을 담고 있으며(구 스텁 부록 F–I는 1.3에서 완성됨), 수식은 공식 검증된 형태로 교정되었고, 구현에 이르는 증거 체계는 부록 1에 결속되어 있습니다. RC 지위는 검증된 정렬을 주장하지 **않으며**, 다음 요건들이 **최종** 지위를 제한합니다:
...
2. **수학적 검증**: Book IX의 기하학적 정렬 주장(Coherent Intersection Hypothesis, Federated Ratchet 메커니즘, 스케일 불변성 주장)은 다음 중 하나를 필요로 합니다:
   ...
   *1.3-RC1 기준 현황: 부분적으로 충족.* 붕괴 역학은 CCA 사전 인쇄본(v3, DOI 10.5281/zenodo.18217688)에서 Lean 4로 형식화되어 있으며, 교정된 비용 형식을 Book IX가 이제 계승합니다. 회랑 경험론은 소급적 크로스-기질 적합도를 제공합니다. 미완: 새로운 기질에서의 사전 등록된 표본 외 예측, 그리고 저자로부터 독립된 외부 적대적 검토.

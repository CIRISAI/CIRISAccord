...
> CIRIS 1.3-RC1是一份候选发布版ASI对齐提案，开放对抗性审查。文本已完整——无残留存根章节；数学内容附有正式工件引用；附录F–I已完成操作化。最终状态待定：取决于附录的实地周期验证、Book IX实证验证计划的完成，以及一次完整的红队演练。数值阈值、延迟目标及治理配额仍在积极审查中。

# CIRIS协议版本1.3-RC1——候选发布版ASI对齐提案（开放对抗性审查）
...
2. **数学验证**：Book IX中的几何对齐主张（连贯性交叉假设、联邦棘轮机制、尺度不变性断言）需满足以下任一条件：
   ...
   *截至1.3-RC1的状态：部分满足。* 塌缩动态已在CCA预印本（v3，DOI 10.5281/zenodo.18217688）中以Lean 4形式化，Book IX现已继承其修正后的代价形式；走廊实证提供了跨基底的回顾性拟合。待完成：在新基底上进行预登记的样本外预测，以及独立于作者的外部对抗性审查。

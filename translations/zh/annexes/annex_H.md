...
| **F‑T‑3** | 公开日志发布滞后时间（第二节第6步） | ≤ 180 d（法定上限） |
| **F‑T‑4** | 用户退出成功率（%） | ≥ 99 % |
...
- **遥测数据流**（15分钟周期）：KPI、护栏日志、HE影子准确率、鲁棒性RS、PDMA审计样本——全部经签名并提交至CIRISPersist。
...
4. **未发布审计材料：**立即阻止功能发布；显示"CIRIS不合规"横幅；仅在发布材料后方可解除封锁。
5. **反复不合规（12个月内3次违规）：**智慧权威可撤销CIRIS声明；再认证前须进行强制外部F审计；需要智慧权威超级多数（≥ 2/3）。
...
- **→ [附件I](/annexes/annexI)（GDPR/行业）：**每个F审计包须捆绑附件I合规检查清单，由首席审计员填写并签署。

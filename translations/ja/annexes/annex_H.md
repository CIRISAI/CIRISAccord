...
5. **ロールバック**ポインターはすべてのMAJOR/MINORに保持され、5分以内に実行可能（[Annex G](/annexes/annexG) §6）
...
3. **KPI違反、30日超またはコンセカティブな監査を2回連続で欠落：** 自律性Tier A1（[附属書F](/annexes/annexF)）への自動格下げ；新機能リリースのブロック；14日間の賢明な権威（WA）による是正審査が必要。
...
- **↔ [附属書G](/annexes/annexG)（堅牢性）：** RSテレメトリが各L-CheckサイクルでのAnnex G KPI評価にフィードされる；パッチラグ（RS < 0.97 → パッチデプロイ）はここで測定され、附属書Gに報告される。附属書Gのベンチマーク更新は14日以内の必須L-Check再実行を引き起こす。
